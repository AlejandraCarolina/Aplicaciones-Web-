        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('cotizaciones.update', $cotizacione->id_cotizacion) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" id="fecha_cot" name="fecha_cot" class="w-full border-gray-300 rounded" value="{{ $cotizacione->fecha_cot}}" required>
                        </div>
                        <div class="mb-4">
                            <label for="vigencia" class="block text-gray-700 font-bold">Vigencia:</label>
                            <input type="date" id="vigencia" name="vigencia" class="w-full border-gray-300 rounded" value="{{ $cotizacione->vigencia}}" required>
                        </div>
                        <div class="mb-4">
                            <label for="cantidad" class="block text-gray-700 font-bold">Cantidad:</label>
                            <input type="number" id="cantidad" name="cantidad" class="w-full border-gray-300 rounded" value="{{ $cotizacione->cantidad}}" required>
                        </div>
                        <div class="mb-4">
                            <label for="comentarios" class="block text-gray-700 font-bold">Comentarios:</label>
                            <input type="text" id="comentarios" name="comentarios" class="w-full border-gray-300 rounded" value="{{ $cotizacione->comentarios}}" required>
                        </div> 
